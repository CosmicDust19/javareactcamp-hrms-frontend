...
export default class CandidateCvService {
    ...
    getById(cvId) {
        return axios.get(`http://javareactcamp-hrms-backend.herokuapp.com/api/cvs/get/byId?cvId=${cvId}`)
    }

    add(values) {
        return axios.post("http://javareactcamp-hrms-backend.herokuapp.com/api/cvs/add", values)
    }

    deleteById(cvId) {
        return axios.delete(`http://javareactcamp-hrms-backend.herokuapp.com/api/cvs/delete/byId?cvId=${cvId}`)
    }

    updateTitle(cvId, title) {
        return axios.put(`http://javareactcamp-hrms-backend.herokuapp.com/api/cvs/update/title?cvId=${cvId}&title=${title}`)
    }

    updateCoverLetter(cvId, coverLetter) {
        if (!coverLetter || coverLetter === "") return axios.put(`http://javareactcamp-hrms-backend.herokuapp.com/api/cvs/update/coverLetter?cvId=${cvId}`)
        return axios.put(`http://javareactcamp-hrms-backend.herokuapp.com/api/cvs/update/coverLetter?coverLetter=${coverLetter}&cvId=${cvId}`)
    }

    updateImg(imgId, cvId) {
        return axios.put(`https://javareactcamp-hrms-backend.herokuapp.com/api/cvs/update/image?cvId=${cvId}&${imgId ? `imgId=${imgId}` : ""}`)
    }
