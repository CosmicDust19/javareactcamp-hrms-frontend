...
export default class CandidateService {
    ...
    updateGithubAccount(candId, link) {
        return axios.put(`https://javareactcamp-hrms-backend.herokuapp.com/api/candidates/update/githubAccount?candId=${candId}${link ? `&githubAccount=${link}` : ""}`)
    }

    updateLinkedinAccount(candId, link) {
        return axios.put(`https://javareactcamp-hrms-backend.herokuapp.com/api/candidates/update/linkedinAccount?candId=${candId}${link ? `&linkedinAccount=${link}` : ""}`)
    }
